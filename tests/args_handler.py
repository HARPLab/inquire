...
class ArgsHandler():
    ...
    def setup_agents(self):
        ...
        if self._args.agent_name == "titrated":
            ...
            ddddd = FixedInteractions(sampling_method, sampling_params, self._args.num_w_samples, self._args.num_traj_samples, [Modality.DEMONSTRATION]*5)
            ddddp = FixedInteractions(sampling_method, sampling_params, self._args.num_w_samples, self._args.num_traj_samples, [Modality.DEMONSTRATION]*4 + [Modality.PREFERENCE])
            dddpp = FixedInteractions(sampling_method, sampling_params, self._args.num_w_samples, self._args.num_traj_samples, [Modality.DEMONSTRATION]*3 + [Modality.PREFERENCE]*2)
            ddppp = FixedInteractions(sampling_method, sampling_params, self._args.num_w_samples, self._args.num_traj_samples, [Modality.DEMONSTRATION]*2 + [Modality.PREFERENCE]*3)
            dpppp = FixedInteractions(sampling_method, sampling_params, self._args.num_w_samples, self._args.num_traj_samples, [Modality.DEMONSTRATION] + [Modality.PREFERENCE]*4)

...
cache=""
for j in $alphas; do
		#name="dempref--${domain}_alpha-${j}"
		#echo $name
		#python tests/corl22.py --domain ${domain} $cache -I 1000 -N 1000 --queries 20 --tests 10 --runs 10 --beta $demo_beta --alpha $j --convergence_threshold 0.01 --output_name $name --agent dempref -V
		name="demo--${domain}_alpha-${j}"
		echo $name
		python tests/corl22.py --domain ${domain} $cache -I 1000 -N 1000 --queries 20 --tests 10 --runs 10 --beta $demo_beta --alpha $j --convergence_threshold 0.01 --output_name $name --agent demo-only -V
    ...
    if [ "$domain" != "linear_combo" ]; then
				#name="dempref--static_${domain}_alpha-${j}"
				#echo $name
				#python tests/corl22.py --domain ${domain} $cache -I 1000 -N 1000 --queries 20 --tests 10 --runs 10 --beta $demo_beta --alpha $j --convergence_threshold 0.01 --output_name $name --agent dempref -V
